...
- 严守 MakeCode Arcade 限制：见本项目内 `reference/limits.md`；API 速查见 `reference/arcade-api.md`；踩坑前先看 `reference/pitfalls.md`。
- 不要清空 `assets.json`（必须是合法 JSON，空资源写 `{}`）。
- 不确定 API 时查本项目 `reference/`，不要臆造不存在的 arcade API。
